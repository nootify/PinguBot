...
class Clown(commands.Cog):
    ...
    async def set_clown(self, ctx, mention):
        ...
        try:
            ...
            raise commands.BadArgument("Specified user is not in the server or you didn't @ their username.")
        ...
        if mention in ctx.guild.members:
            if (ctx.guild.id not in self.server_clowns or
                ...
                if mention.nick is None:
                    ...
                    await ctx.send(f"{self.bot.icons['info']} The clown is set to `{mention.nick} ({mention})`.")
            else:
    ...
    async def prepare_clown(self, ctx):
        ...
        # Check if bot is already connected to a voice channel
            ...
            if ctx.author.voice is None:
                ...
                '''
                Custom permission checker implementation is here because...
                1. @commands.bot_has_guild_permissions(...) decorator only checks if the bot has this permission
                   in one of the roles it has in the SERVER, and the
                2. @commands.bot_has_permissions(...) decorator checks the permissions in the current TEXT channel,
                   but not the actual voice channel.
                '''
                ...
                if self_permissions.connect and self_permissions.speak:
                    ...
                    raise commands.BadArgument(
                        f"I'm missing the `{'`, `'.join(perms for perms, val in required_perms.items() if not val)}`"
                        f" permission(s) for the voice channel.")
